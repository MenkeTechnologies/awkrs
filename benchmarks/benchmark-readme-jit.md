...
- **Generated at (UTC):** 2026-04-09 19:38:23
...
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 6.4 ± 0.6 | 5.0 | 8.5 | 1.01 ± 0.18 |
| `awkrs (bytecode only)` | 6.3 ± 0.9 | 4.8 | 9.9 | 1.00 |

## 2. CPU-bound BEGIN (no input)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 13.6 ± 1.0 | 11.9 | 16.7 | 1.67 ± 0.19 |
| `awkrs (bytecode only)` | 8.1 ± 0.7 | 6.7 | 10.2 | 1.00 |

## 3. Sum first column: `{ s += $1 } END { print s }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 6.5 ± 0.7 | 5.0 | 8.9 | 1.10 ± 0.18 |
| `awkrs (bytecode only)` | 5.9 ± 0.7 | 4.9 | 8.9 | 1.00 |

## 4. Multi-field print: `{ print $1, $3, $5 }` (five fields per line)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 14.3 ± 0.9 | 12.8 | 17.4 | 1.00 |
| `awkrs (bytecode only)` | 14.3 ± 0.8 | 12.8 | 17.1 | 1.00 ± 0.08 |

## 5. Regex filter: `/alpha/ { c += 1 } END { print c }` (lines have no `alpha`)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 5.8 ± 0.7 | 4.7 | 8.1 | 1.01 ± 0.18 |
| `awkrs (bytecode only)` | 5.7 ± 0.7 | 4.4 | 8.4 | 1.00 |

## 6. Associative array: `{ a[$5] += 1 } END { for (k in a) print k, a[k] }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 62.3 ± 3.8 | 56.1 | 72.9 | 1.42 ± 0.13 |
| `awkrs (bytecode only)` | 44.0 ± 3.2 | 40.9 | 57.0 | 1.00 |

## 7. Conditional field: `NR % 2 == 0 { print $2 }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 8.5 ± 0.6 | 7.4 | 11.2 | 1.00 |
| `awkrs (bytecode only)` | 9.2 ± 0.7 | 7.6 | 12.0 | 1.09 ± 0.11 |

## 8. Field computation: `{ sum += $1 * $2 } END { print sum }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 7.8 ± 1.0 | 6.3 | 13.3 | 1.05 ± 0.17 |
| `awkrs (bytecode only)` | 7.4 ± 0.6 | 6.1 | 9.4 | 1.00 |

## 9. String concat print: `{ print $3 "-" $5 }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 13.8 ± 0.8 | 12.2 | 16.9 | 1.00 |
| `awkrs (bytecode only)` | 14.7 ± 1.0 | 12.2 | 17.9 | 1.07 ± 0.09 |

## 10. gsub: `{ gsub("alpha", "ALPHA"); print }` (no `alpha` in input)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 6.8 ± 0.7 | 5.1 | 9.4 | 1.03 ± 0.18 |
| `awkrs (bytecode only)` | 6.7 ± 0.9 | 5.0 | 9.4 | 1.00 |

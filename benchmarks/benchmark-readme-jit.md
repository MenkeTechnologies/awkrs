...
- **Generated at (UTC):** 2026-04-09 17:35:33
...
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 6.9 ± 0.9 | 5.6 | 10.9 | 1.00 ± 0.15 |
| `awkrs (bytecode only)` | 6.8 ± 0.6 | 5.8 | 9.0 | 1.00 |

## 2. CPU-bound BEGIN (no input)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 15.2 ± 1.2 | 12.8 | 20.8 | 1.69 ± 0.20 |
| `awkrs (bytecode only)` | 9.0 ± 0.8 | 7.4 | 12.5 | 1.00 |

## 3. Sum first column: `{ s += $1 } END { print s }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 13.3 ± 1.1 | 11.5 | 16.7 | 1.03 ± 0.11 |
| `awkrs (bytecode only)` | 12.9 ± 0.8 | 11.6 | 16.0 | 1.00 |

## 4. Multi-field print: `{ print $1, $3, $5 }` (five fields per line)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 18.6 ± 0.9 | 16.9 | 21.4 | 1.00 |
| `awkrs (bytecode only)` | 19.0 ± 1.1 | 17.1 | 21.5 | 1.02 ± 0.07 |

## 5. Regex filter: `/alpha/ { c += 1 } END { print c }` (lines have no `alpha`)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 6.8 ± 0.7 | 5.4 | 9.9 | 1.10 ± 0.14 |
| `awkrs (bytecode only)` | 6.2 ± 0.5 | 5.0 | 8.6 | 1.00 |

## 6. Associative array: `{ a[$5] += 1 } END { for (k in a) print k, a[k] }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 71.6 ± 6.9 | 62.3 | 92.3 | 1.41 ± 0.16 |
| `awkrs (bytecode only)` | 50.8 ± 3.1 | 46.0 | 62.9 | 1.00 |

## 7. Conditional field: `NR % 2 == 0 { print $2 }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 12.4 ± 0.5 | 10.9 | 13.7 | 1.00 |
| `awkrs (bytecode only)` | 12.5 ± 0.6 | 11.1 | 15.2 | 1.01 ± 0.06 |

## 8. Field computation: `{ sum += $1 * $2 } END { print sum }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 16.7 ± 0.9 | 14.6 | 19.3 | 1.00 |
| `awkrs (bytecode only)` | 16.8 ± 0.9 | 15.3 | 20.0 | 1.01 ± 0.08 |

## 9. String concat print: `{ print $3 "-" $5 }`

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 18.3 ± 0.8 | 16.6 | 21.7 | 1.00 |
| `awkrs (bytecode only)` | 18.3 ± 0.8 | 16.7 | 20.5 | 1.00 ± 0.06 |

## 10. gsub: `{ gsub("alpha", "ALPHA"); print }` (no `alpha` in input)

| Command | Mean [ms] | Min [ms] | Max [ms] | Relative |
|:---|---:|---:|---:|---:|
| `awkrs (JIT default)` | 8.2 ± 0.8 | 6.4 | 10.1 | 1.00 ± 0.12 |
| `awkrs (bytecode only)` | 8.2 ± 0.6 | 6.7 | 9.6 | 1.00 |

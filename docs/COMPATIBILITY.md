...
- **Character semantics are UTF-8, not locale-driven**: `length`/`substr`/`index`/`toupper`/`tolower` count and fold Unicode scalar values regardless of `LC_ALL`, so `length("é")` is 1 even under `LC_ALL=C` where gawk reports 2. `-b` selects byte semantics explicitly. `printf "%c"` of a code point above 255 follows the same rule: awkrs emits the UTF-8 encoding (gawk's behaviour in a UTF-8 locale), where gawk and mawk under `LC_ALL=C` truncate to a byte and one-true-awk emits nothing.

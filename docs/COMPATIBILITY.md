...
| `-v var=val` | Yes | Yes | Yes | Yes | **Match** — the value is processed as a string literal, so `-v 's=a\tb\n'` is 4 characters. awkrs stored the raw argument and answered 6; it now runs the value through the lexer's own escape table (`lexer::unescape_assignment_value`) rather than a second copy of the rules. |
...
- **Byte-exact strings are not represented (open gap, not a design choice).** awkrs values hold a Rust `String`, which by construction cannot contain a byte that is not part of valid UTF-8, so any such byte is replaced by `U+FFFD` on the way in. All three references pass arbitrary bytes through untouched. Two repros, both with every reference agreeing against awkrs:
  - Passthrough: on input `a\377b`, `awk '{print}'` emits `61 ff 62 0a` in gawk, mawk and one-true-awk; awkrs emits `61 ef bf bd 62 0a`. A field carrying the byte (`$2` of `a\351b c\377d`) goes the same way.
  - Construction: `printf "%c", 233` emits `e9` in all three; awkrs emits `c3 a9`, and `sprintf("%c",233)` stores the same two bytes.
  - Inspection: on input `a\351b`, `substr($0,2,1)` is the single byte `e9` in all three and `ef bf bd` here; `index($0, "\351")` is `2` in all three and `0` here; `$0 ~ "\351"` is true in all three and false here. `length($0)` is the one that already agrees (`3`), because the replacement character is one character per replaced byte.
  - Program text and `-v`: the substitution happens before the program even runs. `awk '{ print ($0 ~ /\351/) ? "Y" : "N" }'` and the same program via `-f`, and `-v x=a\351b`, all work in gawk, mawk and one-true-awk; awkrs exits **2** — from argv it is the CLI parser (`invalid UTF-8 was detected in one or more arguments`) and from `-f` it is the program-file read (`stream did not contain valid UTF-8`). So the gap is not confined to values: any path that decodes bytes into a Rust `String` refuses them.

  These are one gap, not several. The formatting pipeline in `src/format.rs` is `String`-typed end to end, so `%c` cannot emit a lone high byte — but converting only that path would fix `printf "%c"` while leaving `x = sprintf("%c",233)` wrong, because the value still has nowhere to live. Closing this needs a byte-string value type through `Value`, the field splitter and the string builtins; a partial conversion buys an inconsistency rather than parity, so it is left open and stated here instead. The C-locale byte-indexing entry above depends on the same substrate.

  The size of that conversion has been measured rather than estimated. Replacing the payload of `Value::Str` / `StrLit` / `Regexp` with a byte-string newtype carrying no API at all makes `cargo check --lib` report **170 errors across 12 files** — 202 of the ~300 reported locations in `src/runtime.rs` alone, the rest in `vm.rs`, `vm_builtins.rs`, `gawk_extensions.rs`, `builtins.rs`, `lib.rs`, `procinfo.rs`, `fusevm_host.rs`, `bignum.rs`, `format.rs`, `debugger.rs` and `cli_effects.rs`. Giving the newtype the usual conveniences (`From<String>`, `PartialEq<str>`, `Display`) collapses a large share of those, but that count covers only the *representation*. The semantic half is separate and larger: `length`, `substr`, `index`, `split` with an empty separator, `match`'s `RSTART`/`RLENGTH` and regex matching all have to become byte-oriented in a single-byte locale while staying character-oriented in a UTF-8 one, which is what keeps gawk matched in both (see the character-semantics entry above).

...
| Byte-exact strings | **Match** — values hold a byte string (`AwkStr`), so a byte that is not part of valid UTF-8 survives `$0`, fields, `substr`, `index`, `length`, `toupper`, concatenation, array subscripts, `split` elements, `~`, `printf`/`sprintf` and `print`. See the byte-exact-strings entry in §9 for the verified matrix and the four paths that still render. |

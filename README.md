...
**Cranelift JIT (experimental):** The **`jit`** module uses **Cranelift** + **`cranelift-jit`** to compile eligible bytecode chunks into native code with ABI **`(slots, field_fn, array_field_add, var_dispatch, field_dispatch, io_dispatch, val_dispatch) -> f64`** — seven **`extern "C"`** callback pointers covering field reads, fused array updates, HashMap-path scalar ops, dynamic-field mutations, print side-effects, and multiplexed array/match/signal operations. Callers pass a **`JitRuntimeState`** (mutable **`f64`** slot slice + those seven callbacks). Eligible ops include constants, slot and HashMap-path scalar ops (when numeric), arithmetic and comparisons, jumps and fused loop tests, field access (constant **`PushFieldNum`**, dynamic **`GetField`**, NR/FNR/NF, fused **`AddFieldToSlot`** / **`AddMulFieldsToSlot`**), fused **`ArrayFieldAddConst`**, **`$n`** compound / **`++$n`** / **`$n++`**, **fused print opcodes** (**`PrintFieldStdout`**, **`PrintFieldSepField`**, **`PrintThreeFieldsStdout`**, bare **`print`**), **`MatchRegexp`** pattern tests, **flow signals** (**`Next`**, **`NextFile`**, **`ExitDefault`**, **`ExitWithCode`**), and **general array ops** (**`GetArrayElem`**, **`SetArrayElem`**, **`InArray`**, **`DeleteElem`**, **`DeleteArray`**, **`CompoundAssignIndex`**, **`IncDecIndex`** — numeric keys via **`val_dispatch`**). The **`io_dispatch`** callback handles all fused print opcodes as void side-effects. The **`val_dispatch`** callback multiplexes array operations (key passed as `f64`, stringified in the callback), **`MatchRegexp`** (regex tested against `$0`), and flow signals (set a thread-local flag; the VM translates to **`VmSignal::Next`** / **`ExitPending`** etc. after JIT returns). When **`AWKRS_JIT=1`**, the VM tries the JIT for whole chunks that pass **`is_jit_eligible`** (mirrors numeric slot **`f64`** scratch space; callbacks use thread-local **`Runtime`** / **`CompiledProgram`** / **`VmCtx`** pointers so semantics match the interpreter). Chunks with **`printf`**, string concatenation, user/builtin function calls, for-in iteration, getline, or other unsupported opcodes still use the bytecode loop.

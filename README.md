...
**Cranelift JIT (experimental):** The **`jit`** module uses **Cranelift** + **`cranelift-jit`** to compile eligible **numeric** bytecode chunks into native code with ABI **`(slots: *mut f64, field_fn: extern "C" fn(i32) -> f64) -> f64`**. Callers pass a **`JitRuntimeState`** (mutable **`f64`** slot slice + field callback). Eligible ops include constants, slot load/store and fused slot ops (`GetSlot`, `SetSlot`, `IncrSlot`, `CompoundAssignSlot`, …), arithmetic and comparisons, jumps and fused loop tests, and field access via the callback (constant **`PushFieldNum`**, dynamic **`GetField`**, NR/FNR/NF, and fused **`AddFieldToSlot`** / **`AddMulFieldsToSlot`**). The public helpers are **`try_jit_execute`**, **`try_compile`**, **`JitChunk`**, and the legacy **`JitNumericChunk`** / **`try_compile_numeric_expr`** for pure stack-only expressions. When **`AWKRS_JIT=1`**, the VM tries the JIT for whole chunks that pass **`is_jit_eligible`** (mirrors numeric slot **`f64`** scratch space and uses the same field callback as the interpreter). Chunks with strings, **`print`**, arrays, or other opcodes still use the bytecode loop.

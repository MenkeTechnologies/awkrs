...
**Cranelift JIT (via fusevm):** eligible **numeric** bytecode chunks are lowered to [`fusevm`](https://crates.io/crates/fusevm) — the shared bytecode VM also used by `zshrs` and `strykelang` — which Cranelift-compiles them to native code. awkrs has no JIT module of its own: `src/fusevm_bridge.rs` translates an eligible chunk (`is_fusevm_eligible`) into a `fusevm::Chunk` (`build_numeric_chunk`), runs it on `fusevm::VM`, and writes modified slots back into the awkrs runtime. Set `AWKRS_JIT=0` (or pass `-s`/`--no-optimize`) to force the bytecode interpreter.

**Eligible chunks:** pure-numeric bodies — constants, arithmetic/comparisons, `++`/`--`, jumps and fused loop tests, and scalar slot reads/writes whose values stay numeric. Chunks that touch strings, fields, arrays, regexes, `getline`, `print`, user calls, or builtins are ineligible and run on the bytecode interpreter. `-M`/`--bignum` disables the path entirely (MPFR values can't be represented as `f64` slots).

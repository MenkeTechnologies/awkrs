...
- **`RS`** — newline by default; one (UTF-8) char = literal delimiter; `RS=""` = paragraph mode; multi-char = gawk regex (`RT` is the matched text), where a zero-length match is not a separator, so `RS="z?"` leaves the input as one record. `FIELDWIDTHS` selects fixed-width when non-empty.
...
- **Data:** fields, scalars, associative arrays (`a[k]`, `a[i,j]` with `SUBSEP`), `ARGC`/`ARGV` (set before `BEGIN`; `ARGV[0]` is the executable, `ARGV[1..]` are the operands). `ARGV` is re-read as awk walks the operands rather than snapshotted, so `BEGIN { delete ARGV[1] }` skips that file and rewriting an entry redirects the read. An operand of the form `var=value` (left side a valid identifier) is a **command-line assignment**, not a file: it takes effect at the position it occupies, is a POSIX numeric string, and has its escapes decoded like `-v`; a command line whose operands are all assignments still reads standard input. `FS` (regex when multi-char), `FPAT` (gawk-style: non-empty splits by regex match), `split`/`patsplit` (3rd arg accepts regex; `patsplit` 4-arg form populates `seps`). **POSIX record model:** `NF = n` truncates or extends fields and rebuilds `$0` with `OFS`; `$0 = "…"` re-splits and updates `NF`. **`FS`/`FPAT` from literals:** bytecode may store source `"…"` as an internal literal string, but `cached_fs` sync on read still tracks those assignments. **Whole array in a scalar context** (`print a`, string concat, `printf` args, `~` operands, etc.) is a **fatal runtime error** (gawk-style), not a silent empty string. **Scalars:** uninitialized variables compare like numeric `0` where POSIX expects dual 0/`""`. **String constants vs input:** program string literals are not *numeric strings* for `<`/`<=`/`>`/`>=` the way `$n` can be; arithmetic still uses longest-prefix string→number (`"3.14abc"+0` → `3.14`). **Assignment carries that status:** a field or record *assigned* a plain string stops being a numeric string, so after `$1 = "42"` the test `$1 < 7` is a string compare and is true, while `$1 = 42` and `$1 = $2` stay numeric; splitting always produces numeric-string fields, so `$0 = "42"` leaves `$1` numeric. Rebuilding `$0` from fields (`$n = …`, `NF = …`, `gsub` on `$0`) yields a plain string, matching gawk and one-true-awk (mawk keeps it a numeric string there). **`split("", arr, fs)`** returns `0` (no empty pseudo-field).
...
- **AOP function-call intercepts (awkrs-original; no POSIX/gawk counterpart):** aspect-oriented `before`/`after`/`around` advice on user-defined function calls, ported from [`zshrs`](https://github.com/MenkeTechnologies/zshrs)'s command-intercept engine. `intercept("before"|"after"|"around", pattern, code)` registers advice (returns its ID); `pattern` is an exact name, a `*`/`?` glob (`"draw_*"`, `"_*"`), or `"*"`/`"all"`. Around advice calls `intercept_proceed()` to run the original and return its value (omit it to suppress the call). Advice is awk source evaluated in the live interpreter (no fork), sharing globals/arrays and able to call other user functions; the join point exposes `INTERCEPT_NAME`, `INTERCEPT_ARGS`, `INTERCEPT_CMD`, and (for `after`/`around`) the timing globals `INTERCEPT_MS`/`INTERCEPT_US`. Manage the registry with `intercept_list()`, `intercept_remove(id)`, and `intercept_clear()`.
...
**Which tier a program actually reaches (`--tiers`):** enabling the JIT is not the same as being compiled by it. `awkrs --tiers PROGRAM [FILE…]` runs the program on the fusevm backend and then asks fusevm's own predicates — `is_block_eligible`, `block_jit_is_compiled`, `trace_is_compiled`, `find_jit_region` — which tier took each chunk, reporting `BEGIN`, every per-record rule, and `END` separately. Chunks that reach neither tier list the op kinds responsible, so the output is a diagnosis (what to make native next) rather than a verdict.
...
Coverage spans library unit tests for every module (lexer, parser, format, builtins, vm, compiler, runtime, locale_numeric, cli, cyber_help) and integration suites under `tests/` that exercise the gawk-style additions, the slurped-input path, parallel record behavior, and the full CLI surface. Cross-feature combinations (CSV + ENDFILE, paragraph `RS=""` + getline, `FIELDWIDTHS` + NF reassignment, ...) live in [`tests/cross_feature_integration.rs`](tests/cross_feature_integration.rs). Behavior pinned by a reference awk — strnum relational comparison and which values are numeric strings at all (including what an *assignment* to a field or `$0` does to that status), `exit` status propagation and the `END` block still running when `exit` is raised inside a user function, `break`/`continue` in every loop form and their rejection outside one, `getline` at end of input, paragraph-mode field splitting, the field count of an empty record, `sub` with no match, regex-literal `split` separators, `printf` `%c` and negative `*` precision, `typeof($0)` before the record exists, field assignment and `FS` reassignment on the file-input path, the command-line operand rules (`var=value` assigning rather than naming a file, escape processing of `-v` and operand values, and `ARGV` edits changing which files are read), and fatal exit codes — lives in [`tests/posix_parity_regressions.rs`](tests/posix_parity_regressions.rs) and needs no awk installed.
...
A case is only scored once the *reference* produced an answer. If the reference itself is killed by the timeout it is reported as `ORACLE-FAIL` and counted separately, so a case where both awks hang can never be read as a match. The summary line accounts for every case in the corpus — matched, diverged, skipped by `only`, and oracle failures — and warns when those buckets do not add up, which is what catches a malformed corpus quietly dropping cases rather than silently shrinking the denominator.
...
bash scripts/fuzz_parity.sh -F           # feed the input as a file argument
```

Run `-F` as well as the default. awk reaches a named file and a pipe through different code — awkrs mmaps or slurps a file operand and splits fields lazily, while stdin is streamed — and every case in this corpus *and* in `parity/run_parity.sh` is fed on stdin, which left that whole path unmeasured. Two wrong answers were sitting in it: assigning a field before reading one discarded the rest of the record, and a rule assigning `FS` never affected the next record.

The corpus has two halves. [`scripts/fuzz/probes.awkc`](scripts/fuzz/probes.awkc) is hand-written and covers the semantics where awk implementations actually differ: field splitting (`FS`/`OFS`/`RS`/`ORS`, the single-space rule, regex separators, paragraph mode, the field count of an empty record), number↔string coercion including which values are POSIX *numeric strings*, `OFMT` vs `CONVFMT`, uninitialized values, `printf` conversions with `*` width/precision and `%c` over numeric/string/strnum operands, the `substr`/`index`/`split`/`sub`/`gsub`/`match` family with `RSTART`/`RLENGTH`, dynamic regexes from variables versus regex literals, arrays with `in`/`delete`/`SUBSEP` and multidimensional subscripts — including the invariant that a single non-integral subscript converts through `CONVFMT` identically in *every* operation that takes one, so `a[x] = 1`, `x in a`, `delete a[x]`, `a[x] += 1`, `a[x]++` and `typeof(a[x])` all name the same entry — the same rule one call further out, where a *number* reaching a string builtin or a dynamic-regex position converts through `CONVFMT` too, so `length`, `substr`, `index`, `toupper`, `tolower`, `split`, `match`, `sub`/`gsub` and `~`/`!~` all read `1.23456` under `"%.2f"` as the four characters `1.23`, and re-read it on every use rather than caching it at assignment, all six `getline` forms with their return values — including `getline < "-"` reading standard input — output redirection and `close()` returns, the ordering rules that govern when buffered stdout reaches the terminal (opening an output pipe flushes it; `print > "/dev/stdout"` shares the stream rather than opening a second buffer), the `256 + signo` a signal-killed child reports from `system()` and `close()` alike, a regex `RS` whose pattern can match the empty string (where a zero-length match is not a separator, on both the streaming and file-operand read paths), `system()` flush ordering, `srand()`'s return, `NF`/`NR`/`FNR`/`$0` assignment side effects, and control flow per loop form — `break`/`continue` in `for`, `while`, `do-while` and `for-in`, nested and inside a function, their rejection outside any loop, `next`/`nextfile`, and `exit` with and without a status from `BEGIN`, a rule, `END` and a function body. [`scripts/fuzz/gen_awk.pl`](scripts/fuzz/gen_awk.pl) generates the other half from seeded templates over the same areas, so a divergence reproduces exactly from its seed.

A probe may carry `#--- only <ref>[,<ref>]`. That is **not** a failure allowlist — it marks the few probes where the reference awks disagree with *each other* (`printf "%c", ""`, `"0x10" + 0`, `substr` with a negative start, `close()` on a pipe, `printf "%.0d"` of a non-integral value that truncates to zero, a regex `RS` mixing empty and real matches, whether an output pipe re-flushes stdout at `close()`, and `close("/dev/stdout")`) and records which reference defines the behavior awkrs targets.

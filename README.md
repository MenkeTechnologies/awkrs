...
**Cranelift JIT (experimental):** The **`jit`** module uses **Cranelift** + **`cranelift-jit`** to compile straight-line **numeric** bytecode (`PushNum`, `Add`, `Sub`, `Mul`, `Div`, `Neg`, `Pop`) into native machine code (`extern "C" fn() -> f64`). The library API is **`try_compile_numeric_expr`** / **`JitNumericChunk`**. When **`AWKRS_JIT=1`** is set, the VM **execute** loop skips the interpreter for **whole chunks** that consist only of those ops and leave a single `f64` on the stack. Typical awk programs mix I/O, strings, and control flow, so this path is mostly a foundation; extending the JIT to slots, calls, and branches is the next frontier.

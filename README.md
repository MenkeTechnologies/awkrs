...
**Cranelift JIT (experimental):** The **`jit`** module uses **Cranelift** + **`cranelift-jit`** to compile eligible **numeric** bytecode chunks into native code with ABI **`(slots: *mut f64, field_fn, array_field_add) -> f64`** — pointers to **`extern "C"`** functions **`fn(i32) -> f64`** (field as number), **`fn(u32, i32, f64)`** (fused **`a[$field] += delta`**, matching **`ArrayFieldAddConst`**), and slot storage. Callers pass a **`JitRuntimeState`** (mutable **`f64`** slot slice + those two callbacks). Eligible ops include constants, slot load/store and fused slot ops (`GetSlot`, `SetSlot`, `IncrSlot`, `CompoundAssignSlot`, …), arithmetic and comparisons, jumps and fused loop tests, field access via the field callback (constant **`PushFieldNum`**, dynamic **`GetField`**, NR/FNR/NF, fused **`AddFieldToSlot`** / **`AddMulFieldsToSlot`**), and fused **`ArrayFieldAddConst`**. The public helpers are **`try_jit_execute`**, **`try_compile`**, **`JitChunk`**, and the legacy **`JitNumericChunk`** / **`try_compile_numeric_expr`** for pure stack-only expressions. When **`AWKRS_JIT=1`**, the VM tries the JIT for whole chunks that pass **`is_jit_eligible`** (mirrors numeric slot **`f64`** scratch space; field and array callbacks use the same thread-local **`Runtime`** / **`CompiledProgram`** as the interpreter). Chunks with **`print`**, general array subscripts (non-fused), strings, or other unsupported opcodes still use the bytecode loop.

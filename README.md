...
Measured with [hyperfine](https://github.com/sharkdp/hyperfine) (`--shell none` for spot-checks below). BSD awk (`/usr/bin/awk`), GNU gawk 5.4.0, mawk 1.3.4, awkrs **0.1.13**. [`benchmarks/benchmark-results.md`](benchmarks/benchmark-results.md) is generated by `./scripts/benchmark-vs-awk.sh` (§1–§4; **§4** is **awkrs** JIT default vs `AWKRS_JIT=0` bytecode-only for every awkrs workload in §1–§3: **4a–4b** match §2–§3, **4c–4d** match §1 single-thread and parallel). Other tables below are hand-run on the same class of hardware and should be re-measured after meaningful engine changes.

...
**Cranelift JIT (experimental):** The **`jit`** module uses **Cranelift** + **`cranelift-jit`** to compile eligible **numeric** bytecode chunks into native code with ABI **`(slots, field_fn, array_field_add, var_dispatch) -> f64`** — pointers to **`extern "C"`** functions **`fn(i32) -> f64`** (field as number), **`fn(u32, i32, f64)`** (fused **`a[$field] += delta`**), **`fn(u32, u32, f64) -> f64`** (multiplexed **`GetVar`** / **`SetVar`** / **`IncrVar`** / **`DecrVar`** / **`CompoundAssignVar`** / expression **`IncDecVar`** — see **`JIT_VAR_OP_*`** in **`jit.rs`**), plus slot storage. Callers pass a **`JitRuntimeState`** (mutable **`f64`** slot slice + those three callbacks). Eligible ops include constants, slot and HashMap-path scalar ops (when numeric), arithmetic and comparisons, jumps and fused loop tests, field access via the field callback (constant **`PushFieldNum`**, dynamic **`GetField`**, NR/FNR/NF, fused **`AddFieldToSlot`** / **`AddMulFieldsToSlot`**), fused **`ArrayFieldAddConst`**, and the variable ops above. The public helpers are **`try_jit_execute`**, **`try_compile`**, **`JitChunk`**, and the legacy **`JitNumericChunk`** / **`try_compile_numeric_expr`** for pure stack-only expressions. When **`AWKRS_JIT=1`**, the VM tries the JIT for whole chunks that pass **`is_jit_eligible`** (mirrors numeric slot **`f64`** scratch space; callbacks use thread-local **`Runtime`** / **`CompiledProgram`**, and **`var_dispatch`** uses a thread-local **`VmCtx`** pointer so semantics match **`get_var`** / **`set_var`**). Chunks with **`print`**, general array subscripts (non-fused), strings, or other unsupported opcodes still use the bytecode loop.

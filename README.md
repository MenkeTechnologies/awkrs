...
- **`-o` / `--pretty-print`** and **`-p` / `--profile`** — Emit **awkrs**-specific text (AST pretty-print; wall-clock summary and, with **`-j 1`**, per-record-rule invocation counts). **Not** byte-for-byte compatible with gawk’s **`-o`** or **`-p`** output formats.

...
- **`RS`** — **string** `RS` is supported (default newline, multi-byte delimiter, paragraph mode `RS=""`). **Regex** `RS` is **not** implemented (gawk treats non-literal `RS` as a regex).
- **Gawk depth:** **`PROCINFO`**, **`SYMTAB`**, **`FUNCTAB`**, **`FIELDWIDTHS`**, **`IGNORECASE`**, **`@include`**, indirect calls, `/inet/…`, locale **`dcgettext`**, and **`-M` / MPFR** are still missing or stub-level; see **Language coverage** below for what is wired.
...
- **Records & env:** **`RS`** (string: default newline; multi-byte delimiter; **`RS=""`** paragraph mode—blank-line separated records). **`RT`** is set after each primary read to the delimiter text (newline for default **`RS`**). Regex-valued **`RS`** is not implemented. **`ENVIRON`** is populated from the process environment at startup. **`CONVFMT`** controls number→string coercion (concatenation, **`~`** / **`!~`** when operands are numeric, etc.); **`print`** formats numbers with **`OFMT`** (POSIX). Stub gawk globals: **`PROCINFO`**, **`SYMTAB`**, **`FUNCTAB`** (empty associative arrays), plus **`ERRNO`** / **`ARGIND`** initialized (full gawk semantics are not wired).

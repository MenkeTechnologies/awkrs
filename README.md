...
**Cranelift JIT (experimental):** The **`jit`** module uses **Cranelift** + **`cranelift-jit`** to compile eligible **numeric** bytecode chunks into native code with ABI **`(slots, field_fn, array_field_add, var_dispatch, field_dispatch) -> f64`** — pointers to **`extern "C"`** functions **`fn(i32) -> f64`** (field as number), **`fn(u32, i32, f64)`** (fused **`a[$field] += delta`**), **`fn(u32, u32, f64) -> f64`** (multiplexed **`GetVar`** / **`SetVar`** / **`IncrVar`** / **`DecrVar`** / **`CompoundAssignVar`** / expression **`IncDecVar`** — see **`JIT_VAR_OP_*`** in **`jit.rs`**), **`fn(u32, i32, f64) -> f64`** (dynamic-field **`SetField`** / **`CompoundAssignField`** / **`IncDecField`** — compound and inc/dec reuse **`JIT_VAR_OP_*`**; plain assignment uses **`JIT_FIELD_OP_SET_NUM`**), plus slot storage. Callers pass a **`JitRuntimeState`** (mutable **`f64`** slot slice + those four callbacks). Eligible ops include constants, slot and HashMap-path scalar ops (when numeric), arithmetic and comparisons, jumps and fused loop tests, field access via the field callback (constant **`PushFieldNum`**, dynamic **`GetField`**, NR/FNR/NF, fused **`AddFieldToSlot`** / **`AddMulFieldsToSlot`**), fused **`ArrayFieldAddConst`**, the variable ops above, and **`$n`** compound / **`++$n`** / **`$n++`** when numeric. The public helpers are **`try_jit_execute`**, **`try_compile`**, **`JitChunk`**, and the legacy **`JitNumericChunk`** / **`try_compile_numeric_expr`** for pure stack-only expressions. When **`AWKRS_JIT=1`**, the VM tries the JIT for whole chunks that pass **`is_jit_eligible`** (mirrors numeric slot **`f64`** scratch space; callbacks use thread-local **`Runtime`** / **`CompiledProgram`**, and **`var_dispatch`** / **`field_dispatch`** use a thread-local **`VmCtx`** pointer so semantics match **`get_var`** / **`set_var`** and field read/write). Chunks with **`print`**, general array subscripts (non-fused), strings, or other unsupported opcodes still use the bytecode loop.
...
Measured with [hyperfine](https://github.com/sharkdp/hyperfine) (`--shell none` for spot-checks below). BSD awk (`/usr/bin/awk`), GNU gawk 5.4.0, mawk 1.3.4, awkrs **0.1.12**. [`benchmarks/benchmark-results.md`](benchmarks/benchmark-results.md) is generated by `./scripts/benchmark-vs-awk.sh` (§1–§3); other tables are hand-run on the same class of hardware and should be re-measured after meaningful engine changes.

...
Measured with [hyperfine](https://github.com/sharkdp/hyperfine) (`--shell none` for spot-checks below). BSD awk (`/usr/bin/awk`), GNU gawk 5.4.0, mawk 1.3.4, awkrs **0.1.15**. **Relative** = mean time ÷ **fastest** mean in that table. **awkrs** has two rows: default (**JIT** attempted) vs **`AWKRS_JIT=0`** (**bytecode** only) — refreshed from [`benchmarks/benchmark-readme-jit.md`](benchmarks/benchmark-readme-jit.md) (generated **2026-04-09** UTC by `./scripts/benchmark-readme-jit-vs-vm.sh`; re-run after `cargo build --release` to update). **§6** is a single hyperfine run with all five engines on the same 200 K-line input; **§1–§5** and **§7–§10** still mix an older BSD/gawk/mawk sheet with refreshed awkrs rows unless you re-measure everything together. [`benchmarks/benchmark-results.md`](benchmarks/benchmark-results.md) is from `./scripts/benchmark-vs-awk.sh` (cross-engine §1–§4 plus JIT vs bytecode §4a–§4d).
